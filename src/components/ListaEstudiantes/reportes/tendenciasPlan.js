...
export const generarTendenciasPlanExcel = async (estudiantes, showAlerta, modalidadSeleccionada = 'todas') => {
  try {
    ...
    const datos = [
      ...
      ['DISTRIBUCIÓN INSCRIPCIONES POR MODALIDAD'],
      [''],
      // Información general - Tabla Doble Entrada
      ['═══ INFORMACIÓN GENERAL ═══'],
      ['Métrica', 'Valor'],
      ['─────────────────────────', '─────────────────'],
      ['Total inscripciones', distribucion.total],
      ['Modalidad analizada', modalidadSeleccionada],
      ['Fecha análisis', new Date().toLocaleDateString('es-ES')],
      [''],
      // Distribución por modalidad - Tabla Doble Entrada
      ['═══ DISTRIBUCIÓN POR MODALIDAD ═══'],
      ['Modalidad', 'Inscripciones', 'Porcentaje'],
      ['─────────────────', '──────────────', '────────────'],
      ...(distribucion.modalidades ? distribucion.modalidades.map(modalidad => [
        modalidad.nombre,
        modalidad.cantidad,
        calcularPorcentaje(modalidad.cantidad, distribucion.total)
      ]) : [['Sin datos', 0, '0%']]),
    ...
    // Agregar detalles específicos por modalidad - Tabla Doble Entrada
    if (distribucion.detalleModalidad) {
      Object.entries(distribucion.detalleModalidad).forEach(([modalidad, detalle]) => {
        if (modalidadSeleccionada === 'todas' || modalidad.toLowerCase().includes(modalidadSeleccionada.toLowerCase())) {
          datos.push([`═══ ${modalidad} - DETALLE ═══`]);
          
          if (modalidad === 'PRESENCIAL') {
            datos.push(['Año/Curso', 'Inscripciones', 'Porcentaje']);
            datos.push(['────────────', '──────────────', '────────────']);
          } else {
            datos.push(['Plan', 'Inscripciones', 'Porcentaje']);
            datos.push(['────────────', '──────────────', '────────────']);
          }
          
          Object.entries(detalle).forEach(([categoria, cantidad]) => {
            datos.push([
              categoria || 'Sin especificar',
              cantidad,
              calcularPorcentaje(cantidad, distribucion.totalPorModalidad[modalidad] || cantidad)
    ...
    // Observaciones - Tabla Doble Entrada
    datos.push(['═══ OBSERVACIONES ═══']);
    datos.push(['Número', 'Observación']);
    datos.push(['────────', '──────────────────────────────────────────']);
    if (distribucion.observaciones && distribucion.observaciones.length > 0) {
      distribucion.observaciones.forEach((obs, index) => {
        datos.push([index + 1, obs]);
      });
    } else {
      datos.push(['─', 'Sin observaciones']);
    }
    
    const exito = exportarExcel(datos, 'Distribucion_Inscripciones_Modalidad', `DISTRIBUCIÓN INSCRIPCIONES - ${modalidadSeleccionada.toUpperCase()}`);
